...
public class ConnectionsImpl<T> implements Connections<T> {
    ...
    public boolean subscribeToTopic(String topic, int subscriptionId, int connectionId) {
        ...
        if (active_logins.get(connections.get(connectionId).getUser().getUsername()).getSubscribedTopics()
                .containsValue(topic)) {
            return false;
        }
        ...
        active_logins.get(connections.get(connectionId).getUser().getUsername()).subscribeToTopic(topic,
                subscriptionId);
        return true;
    }
    ...
    public void send(String channel, T msg) {
        ...
        for (Integer connectionId : topics.get(channel)) {
            ...
            send(connectionId,
                    connections.get(connectionId).getProtocol().addSubscriptionIdToMessage(msg, subscriptionId));
        }
    }
    ...
    public boolean isUserSubscribedToTopic(int connectionId, String topic) {
        return active_logins.get(connections.get(connectionId).getUser().getUsername()).getSubscribedTopics()
                .containsValue(topic);
    }
